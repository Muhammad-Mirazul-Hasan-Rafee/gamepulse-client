...
const MyReviewGamesCard = ({ reviewedGame }) => {
...
<div className="max-w-full w-full md:w-[600px] bg-base-100 shadow-md rounded-xl p-4 mx-auto">
  ...
  <div className="flex flex-col sm:flex-row gap-4">
    ...
    <div className="flex justify-end gap-4 ml-4 sm:w-1/3">
      <button className="h-12 text-xl p-2 bg-gray-800 text-yellow-300 rounded-lg hover:scale-110 transition">
        <FaEdit />
      </button>
      {/* id must pass korte hbe */}
      <button onClick={()=>handleDelete(_id)} className="h-12 text-xl p-2 bg-red-700 text-white rounded-lg hover:scale-110 transition">
        <FcDeleteRow />
      </button>
    </div>
  </div>
